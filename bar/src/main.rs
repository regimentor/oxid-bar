...
use wayland_client::{
    protocol::{
        ...
        wl_surface::{Event as SurfaceEvent, WlSurface},
    },
...
fn main() {
    ...
    // проверяем, что всё нужное нашлось
    let compositor = state.compositor.as_ref().expect("wl_compositor не найден");
    let layer_shell = state
        .layer_shell
        .as_ref()
        .expect("zwlr_layer_shell_v1 не найден");
    let _shm = state.shm.as_ref().expect("wl_shm не найден");
    let output = state.output.as_ref(); // может быть None (тогда будет на всех мониторах)

    // создаём wl_surface
    let surface: WlSurface = compositor.create_surface(&qh, ());
    state.surface = Some(surface.clone());

    // создаём layer-surface (наш бар)
    let height: u32 = 30;
    ...
    // настраиваем anchor: сверху, по всей ширине
    layer_surface.set_anchor(Anchor::Top | Anchor::Left | Anchor::Right);

    // занимаем полную ширину (width=0), фиксированная высота
    layer_surface.set_size(0, height);

    // exclusive_zone = высота панели, чтобы окна не лезли под неё
    layer_surface.set_exclusive_zone(height as i32);

    state.layer_surface = Some(layer_surface);

    // первый commit – запускаем весь процесс configure
    surface.commit();
    ...
    while state.running {
        event_queue
            ...
            .expect("Ошибка при обработке событий (loop)");
    }
